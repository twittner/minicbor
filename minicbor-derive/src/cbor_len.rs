...
use crate::{Mode, add_bound_to_type_params, collect_type_params};
...
fn on_enum(inp: &mut syn::DeriveInput) -> syn::Result<proc_macro2::TokenStream> {
        ...
        .zip(&variants.attrs)
    {
        ...
        blacklist_enc.add(HashSet::from(blacklist_is_nil_params(
            &inp.generics,
            &fields,
        )));
        let con = &var.ident;
    ...
    let steps = match encoding {
        ...
        Encoding::Array => {
            ...
            for field in fields.fields() {
                ...
                let n: usize = field.index.val().try_into().map_err(|_| {
                    syn::Error::new(field.orig.span(), "index does not fit into usize")
                })?;
                let cbor_len = cbor_len(field.attrs.cbor_len(), field.attrs.codec());
                let is_nil = is_nil(&field.typ, field.attrs.codec());
                let ident = &field.ident;
...
fn blacklist_is_nil_params(generics: &syn::Generics, fields: &Fields) -> Blacklist {
    ...
    blacklist.add({
        let mut with_is_nil = collect_type_params(
            generics,
            fields
                .fields()
                .filter(|f| f.attrs.codec().map(|c| c.is_is_nil()).unwrap_or(false)),
        );
        let without_is_nil = collect_type_params(
            generics,
            fields
                .fields()
                .filter(|f| f.attrs.codec().map(|c| !c.is_is_nil()).unwrap_or(true)),
        );
