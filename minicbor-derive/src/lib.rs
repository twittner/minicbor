...
//!
//! ### `encode_bound`, `decode_bound` and `bound`
//!
//! Fields in structs and enum variants may be annotated with
//! **`#[cbor(encode_bound = "bound")]`**, **`#[cbor(decode_bound = "bound")]`**
//! or **`#[cbor(bound = "bound")]`** where `"bound"` is a a type variable
//! constraint. `encode_bound` applies only to the derived `Encode` implementation
//! and `decode_bound` to the derived `Decode` implementation, wheras `bound`
//! combines the two. When given, this attribute overrides any implicit bounds
//! generated by `minicbor-derive` and is most useful in conjunction with
//! `encode_with`, `decode_with` or `with`.
//!
//! ### `transparent`
//!
