...
//! contrary to regular `Cow` impl of `Decode` which produces owned values.
//!
//! ## `#[cbor(array)]`
//!
//! Uses a CBOR array to encode the annotated struct, enum or enum variant.
//! When applied to an enum it applies to all its variants but can be overriden
//! per variant.
//!
...
//! used to decode `T`. The function needs to satisfy the following type:
//!
//! ```no_run
...
//!
//! When applied to a field of type `T`, the function denoted by `<path>` will be
//! used to encode `T`. The function needs to satisfy the following type:
//!
//! ```no_run
...
//! ## `#[cbor(index_only)]`
//!
//! Enumerations which do not contain fields may have this attribute attached to them.
//! This changes the encoding to encode only the variant index (cf. section
...
//! a module that contains functions named `encode` and `decode` that satisfies the
