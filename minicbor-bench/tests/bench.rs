...
fn serde_encode() {
    ...
    bench("encode/serde_cbor", || {
        buf.clear();
        book.serialize(&mut serde_cbor::Serializer::new(&mut buf))
            .is_ok()
    });
    ...
    bench("encode/minicbor-serde", || {
        buf.clear();
        book.serialize(&mut minicbor_serde::Serializer::new(&mut buf))
            .is_ok()
    });
}
...
fn serde_decode() {
    ...
    println!();
    bench("decode/serde_cbor", || {
        serde_cbor::from_slice::<AddressBook>(&sample).is_ok()
    });
    bench("decode/cbor4ii", || {
        cbor4ii::serde::from_slice::<AddressBook>(&sample).is_ok()
    });
    bench("decode/minicbor-serde", || {
        minicbor_serde::from_slice::<AddressBook>(&sample).is_ok()
    });
}
