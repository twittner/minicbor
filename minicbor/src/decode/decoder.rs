...
impl<'b> Decoder<'b> {
    ...
    pub fn bool(&mut self) -> Result<bool, Error> {
        match self.read()? {
            0xf4 => Ok(false),
            0xf5 => Ok(true),
            b    => Err(Error::type_mismatch(self.type_of(b)?).at(self.pos - 1).with_message("expected bool"))
        }
    }
    ...
    pub fn u8(&mut self) -> Result<u8, Error> {
        match self.read()? {
            n @ 0 ..= 0x17 => Ok(n),
            0x18           => self.read(),
            0x19           => self.read_slice(2).map(read_u16).and_then(|n| try_as(n, "when converting u16 to u8", self.pos - 1)),
            0x1a           => self.read_slice(4).map(read_u32).and_then(|n| try_as(n, "when converting u32 to u8", self.pos - 1)),
            0x1b           => self.read_slice(8).map(read_u64).and_then(|n| try_as(n, "when converting u64 to u8", self.pos - 1)),
            b              => Err(Error::type_mismatch(self.type_of(b)?).at(self.pos - 1).with_message("expected u8"))
        }
    }
    ...
    pub fn u16(&mut self) -> Result<u16, Error> {
        match self.read()? {
            ...
            0x1a           => self.read_slice(4).map(read_u32).and_then(|n| try_as(n, "when converting u32 to u16", self.pos - 1)),
            0x1b           => self.read_slice(8).map(read_u64).and_then(|n| try_as(n, "when converting u64 to u16", self.pos - 1)),
            b              => Err(Error::type_mismatch(self.type_of(b)?).at(self.pos - 1).with_message("expected u16"))
        }
    }
    ...
    pub fn u32(&mut self) -> Result<u32, Error> {
        match self.read()? {
            ...
            0x1b           => self.read_slice(8).map(read_u64).and_then(|n| try_as(n, "when converting u64 to u32", self.pos - 1)),
            b              => Err(Error::type_mismatch(self.type_of(b)?).at(self.pos - 1).with_message("expected u32"))
        }
    }
    ...
    pub fn i8(&mut self) -> Result<i8, Error> {
        match self.read()? {
            ...
            0x18              => self.read().and_then(|n| try_as(n, "when converting u8 to i8", self.pos - 1)),
            0x19              => self.read_slice(2).map(read_u16).and_then(|n| try_as(n, "when converting u16 to i8", self.pos - 1)),
            0x1a              => self.read_slice(4).map(read_u32).and_then(|n| try_as(n, "when converting u32 to i8", self.pos - 1)),
            0x1b              => self.read_slice(8).map(read_u64).and_then(|n| try_as(n, "when converting u64 to i8", self.pos - 1)),
            n @ 0x20 ..= 0x37 => Ok(-1 - (n - 0x20) as i8),
            0x38              => self.read().and_then(|n| try_to!(n, i8, i8::MAX as u8, "when converting u8 to i8", self.pos - 1)),
            0x39              => self.read_slice(2).map(read_u16).and_then(|n| try_to!(n, i8, i8::MAX as u16, "when converting u16 to i8", self.pos - 1)),
            0x3a              => self.read_slice(4).map(read_u32).and_then(|n| try_to!(n, i8, i8::MAX as u32, "when converting u32 to i8", self.pos - 1)),
            0x3b              => self.read_slice(8).map(read_u64).and_then(|n| try_to!(n, i8, i8::MAX as u64, "when converting u64 to i8", self.pos - 1)),
            b                 => Err(Error::type_mismatch(self.type_of(b)?).at(self.pos - 1).with_message("expected i8"))
        }
    }
    ...
    pub fn i16(&mut self) -> Result<i16, Error> {
        match self.read()? {
            ...
            0x19              => self.read_slice(2).map(read_u16).and_then(|n| try_as(n, "when converting u16 to i16", self.pos - 1)),
            0x1a              => self.read_slice(4).map(read_u32).and_then(|n| try_as(n, "when converting u32 to i16", self.pos - 1)),
            0x1b              => self.read_slice(8).map(read_u64).and_then(|n| try_as(n, "when converting u64 to i16", self.pos - 1)),
            n @ 0x20 ..= 0x37 => Ok(-1 - i16::from(n - 0x20)),
            0x38              => self.read().map(|n| -1 - i16::from(n)),
            0x39              => self.read_slice(2).map(read_u16).and_then(|n| try_to!(n, i16, i16::MAX as u16, "when converting u16 to i16", self.pos - 1)),
            0x3a              => self.read_slice(4).map(read_u32).and_then(|n| try_to!(n, i16, i16::MAX as u32, "when converting u32 to i16", self.pos - 1)),
            0x3b              => self.read_slice(8).map(read_u64).and_then(|n| try_to!(n, i16, i16::MAX as u64, "when converting u64 to i16", self.pos - 1)),
            b                 => Err(Error::type_mismatch(self.type_of(b)?).at(self.pos - 1).with_message("expected i16"))
        }
    }
    ...
    pub fn i32(&mut self) -> Result<i32, Error> {
        match self.read()? {
            ...
            0x1a              => self.read_slice(4).map(read_u32).and_then(|n| try_as(n, "when converting u32 to i32", self.pos - 1)),
            0x1b              => self.read_slice(8).map(read_u64).and_then(|n| try_as(n, "when converting u64 to i32", self.pos - 1)),
            n @ 0x20 ..= 0x37 => Ok(-1 - i32::from(n - 0x20)),
            0x38              => self.read().map(|n| -1 - i32::from(n)),
            0x39              => self.read_slice(2).map(read_u16).map(|n| -1 - i32::from(n)),
            0x3a              => self.read_slice(4).map(read_u32).and_then(|n| try_to!(n, i32, i32::MAX as u32, "when converting u32 to i32", self.pos - 1)),
            0x3b              => self.read_slice(8).map(read_u64).and_then(|n| try_to!(n, i32, i32::MAX as u64, "when converting u64 to i32", self.pos - 1)),
            b                 => Err(Error::type_mismatch(self.type_of(b)?).at(self.pos - 1).with_message("expected i32"))
        }
    }
    ...
    pub fn i64(&mut self) -> Result<i64, Error> {
        match self.read()? {
            ...
            0x1b              => self.read_slice(8).map(read_u64).and_then(|n| try_as(n, "when converting u64 to i64", self.pos - 1)),
            ...
            0x3b              => self.read_slice(8).map(read_u64).and_then(|n| try_to!(n, i64, i64::MAX as u64, "when converting u64 to i64", self.pos - 1)),
            b                 => Err(Error::type_mismatch(self.type_of(b)?).at(self.pos - 1).with_message("expected i64"))
        }
    }
    ...
    pub fn f16(&mut self) -> Result<f32, Error> {
        let b = self.read()?;
        if 0xf9 != b {
            return Err(Error::type_mismatch(self.type_of(b)?).at(self.pos - 1).with_message("expected f16"))
        }
        let mut n = [0; 2];
    ...
    pub fn f32(&mut self) -> Result<f32, Error> {
        match self.current()? {
            ...
            b => Err(Error::type_mismatch(self.type_of(b)?).at(self.pos).with_message("expected f32"))
        }
    }
    ...
    pub fn f64(&mut self) -> Result<f64, Error> {
        match self.current()? {
            ...
            b => Err(Error::type_mismatch(self.type_of(b)?).at(self.pos).with_message("expected f64"))
        }
    }
    ...
    pub fn char(&mut self) -> Result<char, Error> {
        let p = self.pos;
        let n = self.u32()?;
        char::from_u32(n).ok_or(Error::invalid_char(n).at(p))
    }
    ...
    pub fn bytes(&mut self) -> Result<&'b [u8], Error> {
        ...
        let n = u64_to_usize(self.unsigned(info_of(b))?, self.pos - 1)?;
        self.read_slice(n)
    }
    ...
    pub fn bytes_iter(&mut self) -> Result<BytesIter<'_, 'b>, Error> {
        ...
        match info_of(b) {
            ...
            n  => {
                let len = u64_to_usize(self.unsigned(n)?, self.pos - 1)?;
